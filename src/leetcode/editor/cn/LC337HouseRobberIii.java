...
//leetcode submit region begin(Prohibit modification and deletion)
    ...
    class Solution {
        ...
        private int robMax(TreeNode root) {
            ...
            int money = root.val;
            if (root.left != null) {
                money += robMax(root.left.left) + robMax(root.left.right);
            }
            if (root.right != null) {
                money += robMax(root.right.left) + robMax(root.right.right);
            }
            int result = Math.max(money, robMax(root.left) + robMax(root.right));
            map.put(root, result);
            return result;
        }
    }
